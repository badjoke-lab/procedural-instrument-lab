...
test('autoplay stays stopped if resume settles but AudioContext is still suspended', async ({ page }) => {
  await page.addInitScript(() => {
    class FakeAudioParam {
      value = 0
      setValueAtTime(value: number) { this.value = value; return this }
    ...
    class FakeAudioContext {
      ...
      createOscillator() { return new FakeOscillatorNode() }
      async resume() {
        // Browser/policy edge case: resume settles without reaching running state.
      }
    }
