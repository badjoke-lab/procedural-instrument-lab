...
test('generates a fit proposal without changing the editable melody', async ({ page }, testInfo) => {
  await page.goto('/')
  ...
  // Step 24 only derives a proposal. The editable source and its mechanical compatibility stay unchanged.
  await expect(compatibility.getByRole('status')).toHaveText('Needs fitting before reliable mechanical playback')
  await expect(compatibility.locator('[data-kind="range"]')).toContainText('MIDI 59')
  await expect(page.getByText('Edited tune', { exact: true })).toBeVisible()

  await autoFit.scrollIntoViewIfNeeded()
  await page.screenshot({ path: testInfo.outputPath('runtime-auto-fit.png'), fullPage: true })
})
