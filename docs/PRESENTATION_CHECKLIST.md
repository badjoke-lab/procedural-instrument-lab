...
- visual amplification, if used, changes only mechanically shared geometry/amplitude and not contact/release ordering or an independent timer,
- for the currently exposed speed and gear-ratio ranges, the physical contact window is long enough to occupy at least one rendered frame at 30 fps; event subsampling alone is not sufficient if the entire visible engagement can occur between presented frames,
