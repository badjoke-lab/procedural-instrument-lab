...
export type AutoFitChange =
  ...
  | {
      ...
      toStartBeat: number
      fromDurationBeats: number
      toDurationBeats: number
    }
...
export function autoFitMusicBoxTune(
  document: TuneDocument,
  options: AutoFitOptions,
  config: MusicBoxConfig = DEFAULT_MUSIC_BOX_CONFIG,
): AutoFitResult {
  assertTuneDocument(document)
  if (options.quantizeStep !== null && (!Number.isFinite(options.quantizeStep) || options.quantizeStep <= 0)) {
    throw new Error('quantizeStep must be positive when enabled')
  }
  ...
  notes = notes.map((note) => {
    ...
    if (options.octaveMoves) {
      ...
      if (target !== null) {
        changes.push({ kind: 'octave', noteId: note.id, fromPitch: note.pitch, toPitch: target })
    ...
    if (options.nearestNoteMapping) {
      ...
      if (target !== null) {
        changes.push({ kind: 'nearest', noteId: note.id, fromPitch: note.pitch, toPitch: target })
        return { ...note, pitch: target }
      }
    }
  ...
  if (options.quantizeStep !== null) {
    notes = notes.map((note) => quantizeNote(note, document.lengthBeats, options.quantizeStep as number, changes))
  }

  if (options.simplifyDenseRepeats) {
    notes = simplifyPinSpacing(notes, document.lengthBeats, config, changes)
  }

  const fitted: TuneDocument = {
    ...document,
    id: document.id.endsWith('-fit') ? document.id : `${document.id}-fit`,
  ...
  return {
    ...
    document: fitted,
    changes,
    compatibility: analyzeMusicBoxCompatibility(fitted, config),
  }
}
...
function findOctaveTarget(pitch: number, supportedNotes: number[]): number | null {
  const targetClass = pitchClass(pitch)
  const candidates = supportedNotes
    .filter((candidate) => pitchClass(candidate) === targetClass)
    .sort((a, b) => Math.abs(a - pitch) - Math.abs(b - pitch) || a - b)
  return candidates[0] ?? null
}

function findNearestTarget(pitch: number, supportedNotes: number[]): number | null {
  const candidates = [...supportedNotes].sort((a, b) => Math.abs(a - pitch) - Math.abs(b - pitch) || a - b)
  return candidates[0] ?? null
}

function quantizeNote(
  note: TuneDocumentNote,
  lengthBeats: number,
  step: number,
  changes: AutoFitChange[],
): TuneDocumentNote {
  ...
  const durationBeats = Math.min(
    availableDuration,
    Math.max(minimumDuration, roundToStep(note.durationBeats, step)),
  )

  if (Math.abs(startBeat - note.startBeat) > EPSILON || Math.abs(durationBeats - note.durationBeats) > EPSILON) {
    changes.push({
      kind: 'quantize',
      noteId: note.id,
      fromStartBeat: note.startBeat,
      toStartBeat: startBeat,
      fromDurationBeats: note.durationBeats,
      toDurationBeats: durationBeats,
    })
  }
...
function simplifyPinSpacing(
  ...
  for (const pitch of config.notes) {
    if (!supported.has(pitch)) continue
    const lane = notes
    ...
    for (const note of lane) {
      ...
      if (!previous || note.startBeat - previous.startBeat + EPSILON >= minimumBeatGap) {
        kept.push(note)
      } else {
  ...
  for (const note of notes) {
    if (removeIds.has(note.id)) {
      changes.push({ kind: 'remove', noteId: note.id, pitch: note.pitch, reason: 'pin-spacing' })
    }
  }
