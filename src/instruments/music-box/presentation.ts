 ...
 *
 * This does not invent a visual-only animation. The returned phase is sampled from the same contact
 * solver used by release/audio. The next frame continues from that presented mechanical phase toward
 * the still-requested target, so release cannot precede a visible engaged frame.
 */
