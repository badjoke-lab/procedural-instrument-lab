...
const NONE: AutoFitOptions = {
  octaveMoves: false,
  nearestNoteMapping: false,
  quantizeStep: null,
  simplifyDenseRepeats: false,
}
...
describe('music-box Auto Fit', () => {
  it('moves out-of-range notes by octaves when their pitch class exists on the comb', () => {
    const source = tune([
      { id: 'b3', pitch: 59, startBeat: 0, durationBeats: 0.5 },
      { id: 'c6', pitch: 84, startBeat: 1, durationBeats: 0.5 },
    ])
  ...
  it('maps remaining pitches to the nearest comb note with a deterministic lower-note tie break', () => {
    const result = autoFitMusicBoxTune(tune([
      { id: 'c-sharp', pitch: 61, startBeat: 0, durationBeats: 0.5 },
    ]), { ...NONE, nearestNoteMapping: true })
  ...
  it('quantizes start and duration without allowing notes to leave the document', () => {
    const result = autoFitMusicBoxTune(tune([
      { id: 'timing', pitch: 60, startBeat: 0.37, durationBeats: 0.38 },
      { id: 'ending', pitch: 62, startBeat: 3.82, durationBeats: 0.18 },
    ]), { ...NONE, quantizeStep: 0.25 })
  ...
  it('simplifies repeated same-lane pins until the configured spacing becomes playable', () => {
    const source = tune([
      { id: 'first', pitch: 60, startBeat: 0, durationBeats: 0.25 },
      { id: 'collision', pitch: 60, startBeat: 0.01, durationBeats: 0.25 },
      { id: 'later', pitch: 60, startBeat: 4, durationBeats: 0.25 },
    ], 8)
  ...
  it('combines transforms into a proposal without changing the source document', () => {
    const source = tune([
      { id: 'range', pitch: 59, startBeat: 0.12, durationBeats: 0.31 },
      { id: 'c4', pitch: 60, startBeat: 0.12, durationBeats: 0.31 },
    ])
    const snapshot = structuredClone(source)
    const result = autoFitMusicBoxTune(source, {
      octaveMoves: true,
  ...
  it('rejects invalid quantization steps', () => {
    expect(() => autoFitMusicBoxTune(tune([
      { id: 'a', pitch: 60, startBeat: 0, durationBeats: 0.5 },
    ]), { ...NONE, quantizeStep: 0 })).toThrow('quantizeStep must be positive')
  })
})
